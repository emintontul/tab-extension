document.addEventListener('DOMContentLoaded', async () => {
  ...
  tabs.forEach(tab => {
    try {
      ...
      // chrome:// veya geçersiz URL'ler için
      const domain = 'other';
      if (!grouped[domain]) {
  ...
  sorted.forEach(([domain, data]) => {
    ...
    countEl.textContent = `${data.tabs.length} tab açık`;

    const closeAllBtn = document.createElement('button');
    closeAllBtn.className = 'close-all-btn';
    closeAllBtn.textContent = 'Hepsini Kapat';
    ...
    data.tabs.forEach(tab => {
      ...
      closeBtn.onclick = async () => {
        ...
        tabEl.remove();
        // Grup boşaldıysa grubu da kaldır
        const remaining = expandedEl.querySelectorAll('.tab-item');
        if (remaining.length === 0) {
          groupEl.remove();
        } else {
          countEl.textContent = `${remaining.length} tab açık`;
        }
      };
